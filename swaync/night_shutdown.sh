...
if [ "$1" = "auto" ]; then
    # Completely silent shutdown while sleeping - no notifications, no sound
    sleep 2
    systemctl poweroff
    exit 0
fi
...
if [ "$1" = "now" ]; then
    notify-send -u normal -a "Night Timer" "🌙 Shutting Down" "Powering off now. Good night!" 2>/dev/null
    sleep 2
    systemctl poweroff
    exit 0
fi
